#!/usr/bin/env python3
'''Benchmark acquisition + preprocess + enrichment with per-phase timings.'''
...
run_acquisition = import_phase("1_acquisition.run").run_acquisition
run_preprocess = import_phase("2_preprocess.run").run_preprocess
...
def main() -> None:
    ...
    print("\n=== Phase 2/3: 2_preprocess ===")
    t0 = time.perf_counter()
    records = run_preprocess(snapshot, acquired)
    ...
    print(f"  Preprocess  : {fmt(t_proc)} ({t_proc:.1f}s)")
